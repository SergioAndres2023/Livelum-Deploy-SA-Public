...
export class RejectDocumentUseCase {
  ...
  async execute(id: string): Promise<DocumentResponse> {
    try {
      this.logger.info('Rejecting document', { id });
      ...
      existingDocument.reject();

      // Guardar los cambios
      await this.documentRepository.update(existingDocument);

      this.logger.info('Document rejected successfully', { id, code: existingDocument.code });

      return this.mapToResponse(existingDocument);
    } catch (error) {
      this.logger.error('Error rejecting document', { error: (error as Error).message, id });
      throw error;
    }
  }
