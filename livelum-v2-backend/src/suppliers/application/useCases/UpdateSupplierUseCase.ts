...
import { UpdateSupplierRequest } from '../dtos/UpdateSupplierRequest';
...
export class UpdateSupplierUseCase {
  ...
  async execute(id: string, request: UpdateSupplierRequest): Promise<SupplierResponse> {
    this.logger.info('Actualizando proveedor', { id, request });
    ...
    try {
      ...
      this.logger.info('Proveedor actualizado exitosamente', { supplierId: supplier.id });
      return this.mapToResponse(supplier);
    } catch (error) {
      this.logger.error('Error al actualizar proveedor', { error: (error as Error).message, id, request });
      throw error;
    }
  }
