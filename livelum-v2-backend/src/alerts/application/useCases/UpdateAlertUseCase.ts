...
import { UpdateAlertRequest } from '../dtos/UpdateAlertRequest';
...
export class UpdateAlertUseCase {
  ...
  async execute(id: string, request: UpdateAlertRequest): Promise<AlertResponse> {
    ...
    try {
      alert.updateInfo(request);
      await this.alertRepository.update(alert);
      return this.mapToResponse(alert);
    } catch (error) {
      throw new Error(`Error al actualizar alerta: ${(error as Error).message}`);
    }
  }
