...
import { CompleteAuditRequest } from '../dtos/CompleteAuditRequest';
...
export class CompleteAuditUseCase {
  ...
  async execute(id: string, request: CompleteAuditRequest): Promise<AuditResponse> {
    try {
      this.logger.info('Completing audit', { id, request });
      ...
      existingAudit.complete(request.actualDate, request.findings, request.recommendations);

      // Guardar los cambios
      await this.auditRepository.update(existingAudit);

      this.logger.info('Audit completed successfully', { id, title: existingAudit.title });

      return this.mapToResponse(existingAudit);
    } catch (error) {
      this.logger.error('Error completing audit', { error: (error as Error).message, id, request });
      throw error;
    }
  }
