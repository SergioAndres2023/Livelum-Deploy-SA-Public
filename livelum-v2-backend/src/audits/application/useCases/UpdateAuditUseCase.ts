...
import { UpdateAuditRequest } from '../dtos/UpdateAuditRequest';
...
export class UpdateAuditUseCase {
  ...
  async execute(id: string, request: UpdateAuditRequest): Promise<AuditResponse> {
    try {
      this.logger.info('Updating audit', { id, request });
      ...
      existingAudit.updateInfo(request);

      // Guardar los cambios
      await this.auditRepository.update(existingAudit);

      this.logger.info('Audit updated successfully', { id, title: existingAudit.title });

      return this.mapToResponse(existingAudit);
    } catch (error) {
      this.logger.error('Error updating audit', { error: (error as Error).message, id, request });
      throw error;
    }
  }
