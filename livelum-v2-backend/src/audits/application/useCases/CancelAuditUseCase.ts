...
export class CancelAuditUseCase {
  ...
  async execute(id: string): Promise<AuditResponse> {
    try {
      this.logger.info('Cancelling audit', { id });
      ...
      this.logger.info('Audit cancelled successfully', { id, title: existingAudit.title });

      return this.mapToResponse(existingAudit);
    } catch (error) {
      this.logger.error('Error cancelling audit', { error: (error as Error).message, id });
      throw error;
    }
  }
