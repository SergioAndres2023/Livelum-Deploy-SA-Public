...
import { UpdateIndicatorRequest } from '../dtos/UpdateIndicatorRequest';
...
export class UpdateIndicatorUseCase {
  ...
  async execute(id: string, request: UpdateIndicatorRequest): Promise<IndicatorResponse> {
    ...
    try {
      indicator.updateInfo(request);
      await this.indicatorRepository.update(indicator);
      return this.mapToResponse(indicator);
    } catch (error) {
      throw new Error(`Error al actualizar indicador: ${(error as Error).message}`);
    }
  }
